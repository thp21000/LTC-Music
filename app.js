...
function urlForView(level,category=null,album=null){
  ...
  u.search='';
  if(level!=='categories'&&category)u.searchParams.set('genre',category);
  if(level==='tracks'&&album)u.searchParams.set('album',album);
  return u.pathname+(u.searchParams.toString()?('?'+u.searchParams.toString()):'')+u.hash;
}
function viewFromUrl(){
  const u=new URL(window.location.href);
  const category=u.searchParams.get('genre');
  const album=u.searchParams.get('album');
  if(category&&album)return {level:'tracks',category,album};
  if(category)return {level:'albums',category,album:null};
  return {level:'categories',category:null,album:null};
}
function setView(level,category=null,album=null,historyMode='push'){
  view={level,category,album};
  query='';
  $('#search').value='';
  const url=urlForView(level,category,album);
...
window.addEventListener('popstate',e=>{
  view=e.state&&e.state.level?e.state:viewFromUrl();
  query='';
  $('#search').value='';
  render();
  window.scrollTo({top:$('#breadcrumb').offsetTop-12,behavior:'smooth'});
